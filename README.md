...
- **`GOOGLE_SHEET_NAME`**: exakter Name des **Tabellenblatts**, in dem die Leads stehen (Default im Code: `Tabellenblatt1`). Wenn die Zeilen z. B. auf **„Leads“** liegen, muss dieser Name in der `.env` stehen – sonst bleibt die Karte leer. Diagnose: `/api/debug/leads-sheet` zeigt `sheetTabs` und `sheetTabMatches`.
